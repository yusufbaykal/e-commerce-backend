...
const updateCart = async (req, res) => {
    ...
    try {
        ...
        let cart = await Cart.findOne({ user_id });
        if (!cart) {
            return res.status(404).json(Response.ErrorResponse(404, "Cart Not Found", "No cart found for the specified user"));
        }

        const itemIndex = cart.items.findIndex(item => item.product_id.toString() === product_id);
        if (itemIndex === -1) {
            return res.status(404).json(Response.ErrorResponse(404, "Item Not Found", "The specified product is not in the cart"));
        }
