    ...
    try {
        ...
        if (validationError) {
            if (validationError.type === 'invalidFields') {
                return res.status(400).json({ status: 400, message: 'Invalid Fields', error: `Invalid fields: ${validationError.keys.join(', ')}` });
            }
            if (validationError.type === 'missingFields') {
                return res.status(400).json({ status: 400, message: 'Missing Fields', error: `Missing fields: ${validationError.keys.join(', ')}` });
            }
        }
